  ...
  return (
    <DashboardLayout role="admin">
      <div className="mb-8">
        ...
        <p className="text-gray-400 mt-2">Pantau stok produk dan lihat siapa saja yang telah membeli.</p>
      </div>

      {/* Summary Cards */}
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6 mb-8">
        <div className="bg-gray-900 border border-white/10 rounded-2xl p-6 flex items-center gap-4">
          <div className="w-12 h-12 bg-blue-500/20 rounded-xl flex items-center justify-center">
            <Package className="w-6 h-6 text-blue-400" />
          </div>
          <div>
            <p className="text-2xl font-bold">{totalStock}</p>
            <p className="text-sm text-gray-400">Total Stok Tersedia</p>
          </div>
        </div>
        <div className="bg-gray-900 border border-white/10 rounded-2xl p-6 flex items-center gap-4">
          ...
          <div>
            ...
            <p className="text-sm text-gray-400">Produk Stok Rendah (≤5)</p>
          </div>
        </div>
        <div className="bg-gray-900 border border-white/10 rounded-2xl p-6 flex items-center gap-4">
          ...
          <div>
            ...
            <p className="text-sm text-gray-400">Produk Habis</p>
          </div>
        </div>
      </div>
      ...
      <div className="bg-gray-900 border border-white/10 rounded-3xl overflow-hidden">
        <div className="p-6 border-b border-white/10">
          ...
          <p className="text-xs text-gray-500 mt-1">Klik baris produk untuk melihat daftar pembeli</p>
        </div>
        <table className="w-full text-left">
          <thead className="bg-black/50 border-b border-white/10">
            <tr>
              ...
              <th className="p-4 font-semibold text-gray-400 text-right">Detail Pembeli</th>
            </tr>
          </thead>
          <tbody>
            {products.map((product) => (
              <Fragment key={product.id}>
                ...
                >
                  <td className="p-4 flex items-center gap-3">
                    {product.image_path ? (
                      ...
                      <div className="w-10 h-10 bg-gray-800 rounded flex items-center justify-center text-[10px] text-gray-500">No Img</div>
                    )}
                  ...
                  <td className="p-4 text-right">
                    <button className="text-gray-400 hover:text-white transition flex items-center gap-1 ml-auto">
                ...
                {expanded === product.id && (
                  <tr key={`buyers-${product.id}`} className="bg-black/30">
                    <td colSpan="5" className="p-6">
                      {buyerLoading && selected === product.id ? (
                        ...
                        <div>
                          <p className="text-sm font-bold mb-3 text-gray-300">
                            Total Terjual: <span className="text-white">{buyers[product.id].total_sold} unit</span>
                          </p>
                            ...
                            <tbody>
                              {buyers[product.id].buyers.map((b, i) => (
                                <tr key={i} className="border-t border-white/5">
                                  <td className="py-2 font-semibold text-white">{b.user_name}</td>
                                  <td className="py-2 text-gray-400">{b.user_email}</td>
                                  <td className="py-2 text-white">{b.quantity}</td>
                                  <td className="py-2 text-gray-400">{b.size || '-'}</td>
                                  <td className="py-2">
                        ...
                        <p className="text-center text-gray-500 italic py-4 text-sm">Belum ada pembeli untuk produk ini.</p>
                      )}
                    </td>
                  </tr>
                )}
            ...
            {products.length === 0 && (
              <tr><td colSpan="5" className="p-8 text-center text-gray-500 italic">Belum ada produk.</td></tr>
            )}
          </tbody>
        </table>
      </div>
