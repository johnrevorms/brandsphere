    ...
    <div className="min-h-screen bg-black text-white pt-24 pb-20">
      <div className="max-w-4xl mx-auto px-4">
        ...
        <button 
          ...
          className="flex items-center gap-2 text-gray-400 hover:text-white transition-colors mb-12 text-sm uppercase tracking-widest font-bold"
        >
          <ArrowLeft className="w-4 h-4" /> Kembali ke Karir
        </button>

        <div className="bg-gray-900 border border-white/10 rounded-3xl p-8 md:p-12">
          <div className="flex flex-col md:flex-row md:items-start justify-between gap-6 border-b border-white/10 pb-8 mb-8">
            ...
            {!user ? (
              ...
              appStatus.status === 'rejected' ? (
                ...
                <button 
                  disabled
                  className="bg-neutral-800 border border-white/20 text-gray-400 px-8 py-3 rounded-full font-bold uppercase tracking-widest flex-shrink-0 cursor-not-allowed"
                >
          ...
          <div className="prose prose-invert prose-lg text-gray-300 max-w-none">
            <h3 className="text-xl font-bold uppercase tracking-widest text-white mb-6">Deskripsi Pekerjaan</h3>
        ...
        {showModal && (
          <div className="fixed inset-0 bg-black/80 backdrop-blur-sm z-50 flex items-center justify-center p-4">
            <div className="bg-neutral-900 border border-white/10 w-full max-w-lg rounded-3xl p-6 sm:p-8 relative shadow-2xl overflow-y-auto max-h-[90vh]">
              <h2 className="text-2xl font-bold italic tracking-wide uppercase mb-6 text-white border-b border-white/10 pb-4">
                Submit Application
              </h2>

              <form onSubmit={handleApply} className="flex flex-col gap-5">
                <div className="flex flex-col gap-1.5">
                  <label className="text-xs font-semibold text-gray-400 uppercase tracking-widest">Nama Lengkap *</label>
                  <input
                    required
                    type="text"
                    value={form.name}
                    onChange={(e) => setForm({ ...form, name: e.target.value })}
                    className="bg-black border border-white/20 rounded-xl p-3 text-white focus:outline-none focus:border-white text-sm transition"
                ...
                <div className="flex flex-col gap-1.5">
                  <label className="text-xs font-semibold text-gray-400 uppercase tracking-widest">No. Telepon / WhatsApp *</label>
                  <input
                    required
                    type="tel"
                    value={form.phone}
                ...
                <div className="flex flex-col gap-1.5">
                  <label className="text-xs font-semibold text-gray-400 uppercase tracking-widest">Unggah CV / Resume (PDF saja, Maks. 5MB) *</label>
                  <input
                    required
                    type="file"
                    accept=".pdf"
                    onChange={(e) => setCvFile(e.target.files[0])}
                    className="bg-black border border-white/20 rounded-xl p-3 text-white focus:outline-none focus:border-white text-sm transition file:mr-4 file:py-1.5 file:px-3.5 file:rounded-full file:border-0 file:text-xs file:font-bold file:bg-white file:text-black hover:file:bg-gray-200 cursor-pointer"
                  />
                </div>

                <div className="flex flex-col gap-1.5">
                  <label className="text-xs font-semibold text-gray-400 uppercase tracking-widest">Cover Letter / Pesan Pengantar</label>
                  <textarea
                    rows={4}
                    value={form.coverLetter}
                    onChange={(e) => setForm({ ...form, coverLetter: e.target.value })}
                    className="bg-black border border-white/20 rounded-xl p-3 text-white focus:outline-none focus:border-white text-sm transition resize-none leading-relaxed"
                ...
                <div className="flex justify-end gap-3 border-t border-white/10 pt-4 mt-2">
                  <button
                    ...
                    className="px-5 py-2.5 rounded-full border border-white/20 text-gray-400 hover:text-white transition text-xs font-bold uppercase tracking-widest"
                  >
                    Batal
                  </button>
                  <button
